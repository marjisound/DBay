<?php confirm_login(); ?>
<?php
include "C:/wamp/www/Db_project/public/notificationClasses.php";
if(logged_in()):
...
global $connection;
if($stmt = $connection -> prepare("SELECT `auction_id`, `item_name`, `seller_id`,
...
if (!($stmt -> fetch())){ // No notifications to show
    ...
    do{
        ...
        if ($watching){
            if ($auctionOver){
                $notifyWatchEnd = new NotifyWatchEnd($auctionID, $itemName, $startPrice,
                                          $reservePrice, $endDate, $viewCount);
                $notifyWatchEnd ->show();
            } else {
                $notifyWatchCont = new NotifyWatchCont($auctionID, $itemName, $endDate,
                                    $viewCount);
                $notifyWatchCont-> show();
            }
        } else {
            if ($auctionOver){
                if ($winning){
                    if ($userHiBid >= $reservePrice){
                        $NotifyWon = new NotifyWon($auctionID, $itemName, $sellerID,
                                      $userHiBid);
                        $NotifyWon -> show();
                    } else {
                        $NotifyTooLow = new NotifyTooLow($auctionID, $itemName, $reservePrice,
                                         $endDate, $userHiBid);
                        $NotifyTooLow -> show();
                } } else {
                    $NotifyLost = new NotifyLost($auctionID, $itemName, $reservePrice,
                                   $endDate, $userHiBid);       
                    $NotifyLost -> show();         }
            } else {
                if ($winning){
...
if($stmt = $connection -> prepare("SELECT `auction_id`, `item_name`,
                                       `start_price`, `reserve_price`,
